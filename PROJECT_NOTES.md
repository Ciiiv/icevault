...
| 16 | eBay Best Offer
| 17 | eBay listing preview modal
| 18 | eBay SOAP proxy
| 19 | eBay token label -- API Keys modal shows "eBay OAuth Token" but Ice Vault uses legacy SOAP Auth'n'Auth token, not OAuth. Rename label to "eBay Legacy Auth'n'Auth Token (SOAP)" and update placeholder text accordingly | -- direct fetch to api.ebay.com/ws/api.dll fails with CORS in browser. Add /proxy/ebay POST endpoint to worker that forwards XML body to eBay SOAP API and returns response. Update ebayQueueSubmitOne and submitToEbay to use WORKER_URL+/proxy/ebay instead of direct eBay URL | ⏯ Needed for eBay listing | -- "Preview Only" button currently shows no visual value. Replace with a modal that renders a rough eBay-style listing preview: card image, title, price, shipping, description formatted as a buyer would see it. Sanity check before submitting | ⏯ Low | -- add Best Offer checkbox to single card eBay tab and eBay Queue shared settings. SOAP field: <BestOfferDetails><BestOfferEnabled>true</BestOfferEnabled></BestOfferDetails> | ⏯ Low |
