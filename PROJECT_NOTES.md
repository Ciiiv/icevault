...
| 16 | eBay Best Offer
| 17 | eBay listing preview modal -- "Preview Only" button currently shows no visual value. Replace with a modal that renders a rough eBay-style listing preview: card image, title, price, shipping, description formatted as a buyer would see it. Sanity check before submitting | ⏯ Low | -- add Best Offer checkbox to single card eBay tab and eBay Queue shared settings. SOAP field: <BestOfferDetails><BestOfferEnabled>true</BestOfferEnabled></BestOfferDetails> | ⏯ Low |
