...
| 15 | API Keys modal polish -- move spend limit warning above all AI key fields and make it generic (not Claude-specific). Star out eBay App ID field (type=password). Currently warning only appears under Anthropic key | ⏯ Low |
...
| 22 | eBay SOAP proxy rate limiting -- /proxy/ebay currently shares /proxy bucket (100/hr) with AI scan calls. Add dedicated /proxy/ebay rate limit at 30/hr to separate listing submissions from AI scan quota | ⏯ Low |
| 22 | eBay SOAP proxy rate limiting -- /proxy/ebay currently shares /proxy bucket (100/hr) with AI scan calls. Add dedicated /proxy/ebay rate limit at 30/hr to separate listing submissions from AI scan quota | ⏯ Low |
| 21 | eBay description cost hint -- cost note above model picker hardcoded to Claude. Should update dynamically per selected model: Claude (~$0.01-0.02 paid), GPT-4o (~$0.01-0.02 paid), Gemini (~$0.001-0.01 free tier) | ⏯ Next |
