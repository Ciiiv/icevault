...
- **eBay tab simplified** — removed batch queue panel and shared settings. eBay tab now shows only cards in eBay Queue collection in the Select Card list. Note above list explains to move cards to eBay Queue to list them. renderEbayCardSelect() filters to EbayQueue collection only, auto-selects first card. No Submit All, no shared settings panel
- **eBay shipping options** — editable shipping cost field added. Dropdown shows service hints. Auto-updates price when changed. USPS Ground Advantage maps to USPSFirstClass in SOAP (USPSGroundAdvantage not valid in SOAP Trading API)
- **eBay description prompt** — updated to be factual and professional. No upsell, no hype, no investment claims, no grade mention for AI-graded cards. Applies to all models (Claude/GPT-4o/Gemini). Under 200 words, facts only
- **eBay condition IDs fixed** — correct ConditionDescriptor values: 400010=Near Mint or Better, 400011=Excellent, 400012=Very Good, 400013=Poor. All 4 tested in production
- **eBay listing end-to-end** — full listing submission working via SOAP proxy. Listing Type (Fixed/Auction), Best Offer, Condition dropdown with ConditionDescriptors (numeric IDs, not ItemSpecifics), Item Location, item specifics (Sport, Player, Season, Card Manufacturer), picture URL from R2. ConditionID 4000=Ungraded. Category 261328. Auth'n'Auth token. Shipping dropdown updated. First successful production listing confirmed and ended
- **eBay listing form improvements** — added Listing Type dropdown (Fixed Price/Auction), Condition dropdown with Near Mint/Excellent/Very Good/Good/Poor, Item Location field (defaults to Detroit MI), Best Offer checkbox, shipping reordered (Top Loader first). Fixed gl undefined error in populateEbayForm. Fixed c undefined in submitToEbay XML. Removed duplicate eBay key fields from form
...
- **AI grade matrix (all 4 sources live)** — Claude, GPT-4o, Gemini, Ximilar all wired up. No more coming-soon tabs. Per-source key check shows "Add [X] key to enable" if key missing. Ximilar maps grades.{final,centering,corners,edges,surface,condition} to standard grade object. Ximilar purpose-built for card grading -- most accurate for condition. Claude/GPT-4o/Gemini better for card identification/OCR. Full breakdown modal shows per-corner/edge/surface detail, centering ratios, autograph detection, weak spots
...
### 🔍 Architecture & Code Quality Findings (June 2026 Review)

| # | Finding | Severity | Status |
|---|---------|----------|--------|
| A1 | XSS via unescaped card data in innerHTML -- c.player, c.brand etc interpolated directly into HTML strings in openCardDetail, renderCardItemHtml, renderSharedView. Low risk for personal use, real vector if public. Fix: add esc() helper | Medium (low risk personal, real if public) | ⏸ If public |
| A2 | bulkExport row map has 23 values but headers array has 24 -- IceVault ID header present, data value missing. Opposite of B1. | Bug | ⏸ Pending |
| A3 | setEbayMode() references ebayModeQueue and ebayModeSingle elements that do not exist in index.html -- queue panel was simplified, these buttons removed. Dead code that would throw if called. | Dead code | ⏸ Pending |
| A4 | _origCapturePhoto assigned but never used -- const _origCapturePhoto=capturePhoto stores original then window.capturePhoto overwrites it. Original never called. | Dead code | ⏸ Pending |
| A5 | Empty setTimeout in openCardDetail -- setTimeout(()=>{// cost hints already set},50) does nothing. | Dead code | ⏸ Pending |
| A6 | renderEbayQueue and ebayQueueSubmitAll reference elements (queueDuration, queueCondition, queueLocation, queueListingType, queueBestOffer, queueShipping, queueShippingCost, queueSubmitProgress) that may not exist in current index.html | Verify | ⏸ Pending |
| A7 | app.js is 3298 lines, single flat file, all globals -- no modules, no single source of truth update pattern. Works at current scale, harder to reason about as it grows. | Tech debt | ⏸ No action needed at personal scale |
...
| eBay SOAP ConditionDescriptors | Card condition for category 261328 uses ConditionDescriptors container not ItemSpecifics. ConditionID 4000=Ungraded, 2750=Graded. Descriptor ID 40001=Card Condition, correct values: 400010=Near Mint or Better, 400011=Excellent, 400012=Very Good, 400013=Poor. All 4 tested and working. Graded cards (2750) require Professional Grader (27501) and Grade (27502) descriptors |
| eBay category 261328 | Sports Trading Card Singles -- replaced old category 214. Required item specifics: Sport=Hockey, Player, Season, Card Manufacturer. Picture URL from R2 included. ConditionID and ConditionDescriptors both required |
...
>
> **Completed May 2026:** Removed duplicate editCardNotes/cancelCardNotes/saveCardNotes functions. Removed duplicate grades: key in saveCard(). Added is_sold + collection_name SQL columns to D1, backfilled existing cards, replaced all JSON LIKE pattern matching with SQL column filters in worker. saveCard() and saveCertCard() refactored to save locally first + reset UI instantly, then upload to R2 and sync to cloud in background (fixes 400 errors from base64 blobs in D1). confirmRescan() now saves grade to correct model slot. Cert card modal shows Grader/Cert#/Official Grade rows. Slab clear now clears all OCR fields. Option A grade field readonly after AI scan. Option B gets clear button, grade format tip, and optional front/back slab photo upload. certGrade changed from span to input throughout. Item 17: eBay listing preview modal -- Preview Only button now opens eBay-styled modal with card image, price, shipping, description tabs, no API calls. Item 23: Reset eBay Listing gold button in card modal -- clears listing status and moves card back to Personal/Graded. eBay form clear button added. clearEbayForm() resets form without auto-selecting a card. Item 14: two-level duplicate detection, ICV label tips modal, other copies row with exact/check badges. normalizeImageToJpeg() fixes PNG/JPEG media type errors on all scan paths. Serial # and team added to graded slab scan prompt + UI + save + clear. Team in PDF card grid. PDF page overlap fixed. OCR prompt strengthened -- read text exactly as printed, do not describe visual appearance. Pencil icon tooltips on all 8 edit-hint spans. R2 image cleanup on card delete + DELETE rate limit 100/hr. Claude model updated to claude-opus-4-6 (better OCR, same price).
>
> **Completed June 2026:** maybePurgeSessions(db) call was missing from worker main handler -- 5% probabilistic global session purge now actually runs (was defined but never called). Orphaned OPENAI PROXY comment removed from worker (was misplaced above image proxy handler). B1 exportCSV IceVault ID header truly fixed (header was present in bulkExport but missing from exportCSV -- row had 24 values, headers had 23). normalizeImageToJpeg() now resizes to 1280px max before saving -- prevents localStorage QuotaExceededError on large camera photos. saveCard() normalizes images before building card object so resized base64 goes to localStorage not raw camera resolution. Ximilar grading fixed: was sending _base64 with full data URL prefix (invalid) + unused Side field -- now sends _url with R2 URLs directly. Ximilar gracefully handles back image detection failure -- pulls grades from front record when overall status 500, shows toast explaining back failed. Smart sync fix: localCount > 0 required to skip full pull -- prevents "Showing 0 results" on fresh/incognito load where localStorage is empty.
>
